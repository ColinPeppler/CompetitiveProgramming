import java.util.*;
public class FastMaximumFlow{
	public static void main(String[] args){
		Scanner in = new Scanner(System.in);
		int numOfVertices = in.nextInt();
		int numOfEdges = in.nextInt();
		
		MaxFlowSolver FF  = new FordFulkerson();

		// Adds nodes to MaxFlowSolver
		for(int vertex  = 0; vertex < numOfVertices; vertex++){
			FF.addNode();
		}

		for(int i = 0; i < numOfEdges; i++){
			// vertices are index 1 based
			int u = in.nextInt()-1;
			int v = in.nextInt()-1;
			int capacity = in.nextInt();
			FF.link(u, v, capacity);
			//FF.link(v, u, capacity);
		}
		//maxFlow(sink, source)
		System.out.println(FF.getMaxFlow(FF.nodes.get(0), FF.nodes.get(numOfVertices-1)));
	}
